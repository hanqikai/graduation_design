...
from facenet_retinaface.nets.facenet import Facenet

from facenet_retinaface.nets_retinaface.retinaface import RetinaFace
from facenet_retinaface.utils.anchors import Anchors
from facenet_retinaface.utils.config import cfg_mnet, cfg_re50
from facenet_retinaface.utils.utils import (Alignment_1, compare_faces, letterbox_image,
                         preprocess_input)
from facenet_retinaface.utils.utils_bbox import (decode, decode_landm, non_max_suppression,
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        #----------------------------------------------------------------------#
        "retinaface_model_path" : r"facenet_retinaface\model_data\Retinaface_Epoch185-Total_Loss6.0680.pth",
        #----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        #----------------------------------------------------------------------#
        "facenet_model_path"    : r"facenet_retinaface\model_data\facenet_mobilenet.pth",
        #----------------------------------------------------------------------#
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load(r"facenet_retinaface\model_data\{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
            self.known_face_names     = np.load(r"facenet_retinaface\model_data\{backbone}_names.npy".format(backbone=self.facenet_backbone))
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        np.save("facenet_retinaface/model_data/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone),face_encodings)
        np.save("facenet_retinaface/model_data/{backbone}_names.npy".format(backbone=self.facenet_backbone),names)
